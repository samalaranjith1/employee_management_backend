...
// Example mocked data usage

function DepartmentPerformanceCards({ cards }) {
  return (
    <Col
      xs={12}
      lg={3}
    ...
    >
      {cards.map((card, index) => {
        ...
        return (
          ...
          >
            <Card.Body>
              <IconComponent className={`${card.iconColorClass} fs-4`} />
              <p className="mb-1 text-muted">{card.title}</p>
              <h5 className={`fw-bold ${card.iconColorClass}`}>{card.value}</h5>
            </Card.Body>
          </Card>
        );
      })}
