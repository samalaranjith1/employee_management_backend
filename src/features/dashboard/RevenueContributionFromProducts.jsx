...
import { revenueContributionFromProductsDataFormatter } from "@/utils/data_formatters/dashboardFormatter";

const RevenueContribution = () => {
  const myScrollRef = useRef(null);
  const { startDate, endDate } = useDashboardContext();

  // Keep only table + tag styles; cards layout will match ConsumptionSummary
  const styles = {
    container: {
      background: "#fff",
      borderRadius: "12px",
  ...
  return (
    <Container fluid style={styles.container} className="p-2">
      ...
      {/* ✅ Use ServiceRenderer (same as ConsumptionSummary) */}
      <ServiceRenderer
        queryHook={useProductsSalesDistribution}
        queryKey={[
          "productsSalesDistribution",
          { startdt: startDate, enddt: endDate },
        ]}
        queryFn={() =>
          useProductsSalesDistribution({
            startdt: startDate,
            enddt: endDate,
          }).queryFn
        }
        ...
        formatter={revenueContributionFromProductsDataFormatter}
        shimmerCount={3}
      >
        {({ topCards, tableData }, refetch) => (
          <>
            {/* 🟩 Top Cards now styled like ConsumptionSummary */}
            <div
              ref={myScrollRef}
              className="d-flex"
              style={{
